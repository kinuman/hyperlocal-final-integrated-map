...
export default function WalletConnect() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      toast.success('ウォレット接続成功！');
    } catch (err: any) {
      console.error('Connection error:', err);
      // user rejected action
      const errorCode = err.code || err.info?.error?.code;
      const errorMessage = err.message || err.info?.error?.message;

      if (errorCode === 4001 || errorMessage?.includes('rejected') || errorMessage?.includes('declined')) {
        const msg = '接続リクエストがキャンセルされました。';
        setError(msg);
        toast.error(msg);
      } else {
        const msg = errorMessage || '接続に失敗しました。';
        setError(msg);
        toast.error(msg);
      }
    } finally {
